...
# Required software
FCGI_CMD_PATH=$(command -v cgi-fcgi) || {
	echo >&2 "Make sure fcgi is installed (i.e. apk add --no-cache fcgi). Aborting."
	exit 4
}
command -v sed 1>/dev/null || {
	echo >&2 "Make sure sed is installed (i.e. apk add --no-cache busybox). Aborting."
	exit 4
}
command -v tail 1>/dev/null || {
	echo >&2 "Make sure tail is installed (i.e. apk add --no-cache busybox). Aborting."
	exit 4
}
command -v grep 1>/dev/null || {
	echo >&2 "Make sure grep is installed (i.e. apk add --no-cache grep). Aborting."
	exit 4
}
...
get_fpm_status() {
	...
	if test "$FPM_STATUS" = "File not found."; then
		printf >&2 "php-fpm status page non reachable\\n"
		exit 8
	fi
}
...
check_fpm_health_by() {
	...
	if test "$VALUE_ACTUAL" -gt "$VALUE_EXPECTED"; then
		printf >&2 "'%s' value '%s' is greater than expected '%s'\\n" "$OPTION" "$VALUE_ACTUAL" "$VALUE_EXPECTED"
		exit 1
	fi
}
...
check_later() {
	...
	if ! test "$2" -eq "$2" 2>/dev/null; then
		printf >&2 "'%s' option value must be an integer, '%s' given\\n" "$1" "$2"
		exit 3
	fi
...
if ! GETOPT=$(getopt -o v --long verbose,accepted-conn:,listen-queue:,max-listen-queue:,listen-queue-len:,idle-processes:,active-processes:,total-processes:,max-active-processes:,max-children-reached:,slow-requests: -n 'php-fpm-healthcheck' -- "$@"); then
	echo >&2 "Invalid options, terminating."
	exit 3
fi
